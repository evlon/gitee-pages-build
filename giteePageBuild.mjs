...
class GiteePage {
    ...
    async login() {
        let giteeDomain = GiteePage.giteeDomain || GITEE_DOMAIN;
        let login_index_url = `${giteeDomain}/login`
        let check_login_url = `${giteeDomain}/check_user_login`
        ...
        let csrf_token = this.get_csrf_token(respText);
        ...
        let data = `${csrf_token}$gitee$${this.password}`

        let clientKey = new NodeRsa(PUBLIC_KEY);
        clientKey.setOptions({ encryptionScheme: 'pkcs1' })

        let encrypt_data = clientKey.encrypt(data, 'base64')
    ...
    async pageBuild() {
        let giteeDomain = GiteePage.giteeDomain || GITEE_DOMAIN;
        let pages_url = `${giteeDomain}/${this.username}/${this.repo}/pages`
        let rebuild_url = `${pages_url}/rebuild`
